...
// Query dasar menyesuaikan filter pencarian jika ada keyword
$query_str = "SELECT * FROM buku WHERE 1=1";
if (!empty($keyword)) {
    $query_str .= " AND (Judul LIKE '%$keyword%' OR Penulis LIKE '%$keyword%' OR Kategori LIKE '%$keyword%')";
}
            ...
            <div class="row g-2 mb-4 d-print-none align-items-center">
                <div class="col-md-5">
                    <form method="POST" action="" class="input-group">
                        <input type="text" name="keyword" class="form-control bg-dark text-white border-secondary form-control-dark" placeholder="Cari Judul atau Kategori/Penulis..." value="<?= htmlspecialchars($keyword) ?>">
            ...
            <div class="row">
                ...
                if ($query_buku && mysqli_num_rows($query_buku) > 0):
                    while($row = mysqli_fetch_assoc($query_buku)): 
                        $nama_file_gambar = !empty($row['Cover']) ? $row['Cover'] : '';

                        if (!empty($nama_file_gambar) && file_exists("cover/" . $nama_file_gambar)) {
                            $src_gambar = "cover/" . $nama_file_gambar;
                        } else {
